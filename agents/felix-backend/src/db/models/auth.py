...
class Organization(AuditMixin, Base):
    ...
    edition = Column(Enum(OrgEdition), nullable=False, default=OrgEdition.co_worker)
    region = Column(Text, nullable=False)
    sso_config = Column("sso_config", Text, nullable=True)  # JSONB stored as text in ORM; use JSON type in migration
    status = Column(Enum(OrgStatus), nullable=False, default=OrgStatus.active)
...
class User(AuditMixin, Base):
    ...
    auth_provider = Column(Enum(UserAuthProvider), nullable=False)
    external_subject = Column(Text, nullable=True)
    password_hash = Column(Text, nullable=True)
    global_role = Column(Enum(UserGlobalRole), nullable=False, default=UserGlobalRole.learner)
...
class Enrolment(AuditMixin, Base):
    ...
    role = Column(Enum(EnrolmentRole), nullable=False, default=EnrolmentRole.learner)
    status = Column(Enum(EnrolmentStatus), nullable=False, default=EnrolmentStatus.active)
