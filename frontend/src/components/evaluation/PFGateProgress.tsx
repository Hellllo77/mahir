...
import type { ExerciseProgress } from "@/lib/api-types";
...
export function PFGateProgress({ progress }: Props) {
  ...
  return (
    <div className="card" style={{ gap: "var(--space-4)", display: "flex", flexDirection: "column" }}>
      ...
      <div className="stack" style={{ gap: "var(--space-3)" }}>
        <ProgressBar
          value={attempts}
          max={gate.min_attempts}
          label="Genuine attempts"
          showFraction
        />
        <ProgressBar
          value={approaches}
          max={gate.min_distinct_approaches}
          label="Distinct approaches tried"
          showFraction
        />
        <div style={{ display: "flex", flexDirection: "column", gap: "var(--space-1)" }}>
          <div style={{ display: "flex", justifyContent: "space-between" }}>
            ...
            <span
              ...
              style={{
                color:
                  seconds >= gate.min_exploration_seconds
                    ? "var(--color-success)"
                    : "var(--color-text-secondary)",
              }}
            >
              {fmtSeconds(seconds)} / {fmtSeconds(gate.min_exploration_seconds)}
            </span>
          </div>
